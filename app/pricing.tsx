...
function PlanCard({ name, price, period, desc, popular, features, cta, ctaFilled }: {
  ...
  return (
    <View style={{
      ...
      borderRadius: 20, padding: r.isXs ? 16 : 20,
      borderWidth: popular ? 2 : 1,
      borderColor: popular ? "#7C3AED" : colors.border,
      shadowColor: popular ? "#7C3AED" : "#000",
      shadowOffset: { width: 0, height: popular ? 8 : 2 },
      shadowOpacity: popular ? 0.15 : 0.05,
      shadowRadius: popular ? 20 : 8,
      ...
      {popular && (
        <View style={{ position: "absolute", top: -14, left: 0, right: 0, alignItems: "center" }}>
          <View style={{
            ...
            <Text style={{ color: "#FFFFFF", fontSize: r.fontSize.xs, fontWeight: "700" }}>Most Popular</Text>
          </View>
        </View>
      )}
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
        <View style={{ flex: 1, minWidth: 0, marginRight: 8 }}>
          <Text style={{ color: colors.foreground, fontSize: r.fontSize.xl, fontWeight: "700" }}>{name}</Text>
          <Text style={{ color: colors.muted, fontSize: r.fontSize.sm, marginTop: 2 }}>{desc}</Text>
        </View>
        <View style={{ flexDirection: "row", alignItems: "flex-end", gap: 2, flexShrink: 0 }}>
          <Text style={{ color: popular ? "#7C3AED" : colors.foreground, fontSize: r.isXs ? 26 : 30, fontWeight: "800" }}>
            {price}
          </Text>
          <Text style={{ color: colors.muted, fontSize: r.fontSize.sm, marginBottom: 5 }}>{period}</Text>
        </View>
      </View>
      <View style={{ gap: 8, marginBottom: 16 }}>
        {features.map((feature) => (
          <View key={feature} style={{ flexDirection: "row", alignItems: "flex-start", gap: 8 }}>
            <IconSymbol name="checkmark.circle.fill" size={16} color="#22C55E" style={{ marginTop: 1, flexShrink: 0 }} />
            <Text style={{ color: colors.foreground, fontSize: r.fontSize.sm, flex: 1, lineHeight: 18 }}>{feature}</Text>
          </View>
        ))}
      </View>
      <TouchableOpacity
        style={{
          backgroundColor: ctaFilled ? "#7C3AED" : "transparent",
          borderRadius: 12, height: 52,
          alignItems: "center", justifyContent: "center",
          borderWidth: ctaFilled ? 0 : 1.5,
          borderColor: ctaFilled ? undefined : colors.border,
        }}
        activeOpacity={0.8}
      >
        <Text style={{ color: ctaFilled ? "#FFFFFF" : colors.foreground, fontSize: r.fontSize.md, fontWeight: "700" }}>
          {cta}
        </Text>
      </TouchableOpacity>
    </View>
    ...
    <ScreenContainer>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        <View style={{
          ...
          <Text style={{ color: colors.foreground, fontSize: r.isXs ? 22 : 26, fontWeight: "800", marginBottom: 4 }}>
            Choose Your Plan
          </Text>
        ...
        <View style={{ paddingHorizontal: r.px, marginTop: 20, gap: 16 }}>
